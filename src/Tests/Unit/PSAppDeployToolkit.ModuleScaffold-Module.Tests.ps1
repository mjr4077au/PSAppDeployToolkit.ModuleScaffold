BeforeAll {
    ...
    #-------------------------------------------------------------------------
    $ModuleName = 'PSAppDeployToolkit.ModuleScaffold'
...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module PSAppDeployToolkit.ModuleScaffold.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain PSAppDeployToolkit.ModuleScaffold.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "PSAppDeployToolkit.ModuleScaffold.psm1"
        } #validPSM1
